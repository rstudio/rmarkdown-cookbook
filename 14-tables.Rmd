...
There are many other R packages that can be used to generated tables. The main reason that I introduced `kable()` (Section \@ref(kable)) and **kableExtra** (Section \@ref(kableextra)) is not that they are better than other packages, but because I'm familiar with only them.^[Frankly speaking, I rarely use tables by myself, so I'm not highly motivated to learn how to create sophisticated tables.] Next I will list the packages that I'm aware of but not very familiar with. You can check them out by yourself, and decide which one fits your purpose best.
...
- **reactable**: Similar to **DT** (also creates interactive tables based on a JavaScript library), but frankly speaking, it looks better than **DT** in several aspects in my eyes (such as row grouping and aggregation, and embedding HTML widgets). Had **reactable** existed in 2015, I would not have developed **DT**. That said, **reactable** does not contain all the features of **DT**, so you may read its documentation and see which one fits your purpose better: https://glin.github.io/reactable/.

- **rhandsontable**: Also similar to **DT**, and has an Excel feel (e.g., you can edit data directly in the table). Visit https://jrowen.github.io/rhandsontable/ to know more about it.

- **pixiedust**: Features creating tables for models (such as linear models) converted through the **broom** package. It supports Markdown, HTML, and LaTeX output formats. Its repository is at https://github.com/nutterb/pixiedust.

- **stargazer**: Features formatting regressions models and summary statistics tables. The package is available on CRAN at https://cran.r-project.org/package=stargazer.

- **xtable**: Perhaps the oldest package for creating tables---the first release was made in 2000. It supports both LaTeX and HTML formats. The package is available on CRAN at https://cran.r-project.org/package=xtable.

I'm not going to introduce the rest of packages, but will just list them here: **tables** (https://cran.r-project.org/package=tables), **pander** (http://rapporter.github.io/pander/), **tangram** (https://github.com/spgarbet/tangram), **ztable** (https://github.com/cardiomoon/ztable), and **condformat** (https://github.com/zeehio/condformat).
