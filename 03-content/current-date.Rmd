...
---
```
This will automatically update each time you Knit your document i.e. `r format(Sys.time(), '%d %B, %Y')`. If we wish to customize the format of the dates, we can alter the time format by providing our own text string. A full table of POSIXct formats is shown in Table @\ref(tab:dateTimeFormats), with some complete examples shown below:
