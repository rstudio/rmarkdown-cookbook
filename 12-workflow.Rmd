...
````md
`r ''````{cat, engine.opts = list(file = fs::path_temp('script.R'))}
...
````md
`r ''````{r, code = readLines(fs::path_temp('script.R'))}
```
````
...
````md
`r ''````{cat, engine.opts = list(file = fs::path_temp('demo.yml'), lang = "yaml")}
a:
  aa: something
  bb: 1
b:
...
````md
`r ''````{cat, engine.opts = list(file = fs::path_temp('demo.yml')), class.source = "yaml"}
a:
  aa: something
  bb: 1
b:
