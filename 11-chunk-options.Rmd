...
There are more than 50 chunk options that can be used to fine-tune the behavior of **knitr** when processing R chunks. Please refer to the online documentation at <https://yihui.org/knitr/options/> for the full list of options. `r if (knitr::is_latex_output()) 'For your convenience, we have also provided a copy of the documentation in Appendix \\@ref(full-options) of this book.'`
...
When the chunk option `cache = TRUE`, cached objects will be lazy-loaded into the R session, which means an object will not be read from the cache database until it is actually used in the code. This can save you some memory when not all objects are used later in the document. For example, if you read a large data object but only used a subset in the subsequent analysis, the original data object will not be loaded from the cache database:
...
In Section \@ref(chunk-styling), we showed some examples of styling source and text output blocks based on the chunk options `class.source` and `class.output`. Actually, there are a wider range of similar options in **knitr**, such as `class.message`, `class.warning`, and `class.error`. These options can be used to add class names to the corresponding text output blocks, e.g., `class.error` adds classes to error messages when the chunk option `error = TRUE` (see Section \@ref(opts-error)). The most common application of these options may be styling the output blocks with CSS rules defined according to the class names, as demonstrated by the examples in Section \@ref(chunk-styling).
...
Please see the help pages of the output format functions (e.g., `?rmarkdown::html_document`) for the possible values for the option `df_print`, or whether an output format supports the `df_print` option.
...
You may wonder why not setting the chunk options directly, e.g., just `knitr::opts_chunk$set(attr.source = '.numberLines')` like we did in Section \@ref(number-lines). The advantage of using the option hooks here is that they only _append_ the attribute `.numberLines` to chunk options, which means they will not _override_ existing chunk option values, e.g., the source code block of the chunk below will be numbered (with the above setup), and the numbers start from the second line:
