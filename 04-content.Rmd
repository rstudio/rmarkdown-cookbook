...
We only introduced one way to generate R package citations above. To dynamically generate citations for other types of literature, you may check out the **knitcitations** package [@R-knitcitations].
...
Certain R Markdown templates will allow you to specify additional parameters directly within the YAML. For example, the [Distill](https://rstudio.github.io/distill/) output format allows `url`, `affiliation`, and `affiliation_url` to be specified. After you install the **distill** package [@R-distill]:
...
We can use **bookdown** [@R-bookdown] output formats to add figure numbers to their captions. Below is an example:
...
You can add this feature to R Markdown output formats outside **bookdown**, too. The key is to use those formats as the "base formats" of **bookdown** output formats. For example, in order to number and cross-reference figures in the `rticles::jss_article` format, you can use:
...
To display the actual math equations, you need the chunk option `results = "asis"` (see Section \@ref(results-asis) for the meaning of this option), otherwise the equations will be displayed as normal text output.
...
When you generate a series of plots in a code chunk, you can combine them into an animation. It is easy to do so if the output format is HTML---you only need to install the **gifski** package [@R-gifski] and then set the chunk option `animation.hook = "gifski"`. Figure \@ref(fig:pacman) shows a simple "Pac-man" animation created from the code chunk below:
...
The R package **animation** [@R-animation] contains several animation examples to illustrate methods and ideas in statistical computing. The **gganimate** package [@R-gganimate] allows us to create smooth animations based on **ggplot2** [@R-ggplot2]. Both packages work with R Markdown.
