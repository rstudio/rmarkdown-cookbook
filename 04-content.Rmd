...
To display the actual math equations, you need the chunk option `results = "asis"`\index{chunk option!asis} (see Section \@ref(results-asis) for the meaning of this option), otherwise the equations will be displayed as normal text output.
...
There is a simple method of extracting all code chunks in a document and put them together in a single code chunk using the chunk option `ref.label`\index{chunk option!ref.label} and the function `knitr::all_labels()`\index{knitr!all\_labels()}, e.g.,
