...
The path you provide to `href` should assume the same relative path structure as you would use to reference any other asset (e.g. an image or data set). For the image itself, most small, square PNG files will work reasonably well Bear in mind that a typical web browser will often display the image in a 16 x 16 pixel box, so simple designs are better. 
...
- [**shinyapps.io**](https://www.shinyapps.io/) allows for hosting dynamic content which requires a server to run R. For example, one can host interactive R Markdown documents which include Shiny components.^[You may include Shiny components in an R Markdown document by setting the option `runtime: shiny` or `runtime: shinyprerendered` in the YAML metadata. You will no longer be able to render your document to an HTML document as before; instead, you run your document with `rmarkdown::run()`. To learn more, please refer to [Section 19 on Shiny Documents](https://bookdown.org/yihui/rmarkdown/shiny-documents.html) of the R Markdown Definitive Guide [@rmarkdown2018] and the documentation about `runtime: shiny_prerendered`.] shinyapps.io is an analog to RPubs for Shiny applications. Apps and interactive R Markdowns can be published using the push-button in the RStudio IDE or the `rsconnect::deployApp()` function. See the user guide (https://docs.rstudio.com/shinyapps.io/index.html) for more details. 
...
- [**RStudio Connect**](https://rstudio.com/products/connect/) is an enterprise product which organizations may run on their own servers. It can host a wide variety of content created in R (R Markdown documents, Shiny apps, APIs) in a secured environment with document-level access controls, viewership history, and more. Similar to the open-source RPubs and shinyapps.io offerings, content can be published to RStudio Connect using manual upload, the **rsconnect** package, or with Git-based deployment. 

### Static website services

In a few words, a simple static website is composed of an `index.html` (the main page), any number of other HTML, JavaScript, CSS files, and additional content such as images. This collection of files can be hosted as-is on a web server and rendered in a browser. 

When R Markdown is rendered to the HTML output format, the result may be treated as a static website.
Websites can range in complexity from a single, standalone HTML file (which is what we get when we use the default `self_contained: true` option), a set of files, or a complex project like **blogdown** output (which relies upon a static website generator). See [Section 2.1](https://bookdown.org/yihui/blogdown/static-sites.html) in [@blogdown2017].
...
- [**Netlify**](https://www.netlify.com/) is a platform to build and deploy static website content. It is a popular choice for web content created by the **blogdown** and **pkgdown** packages, but it can host all types of markdown and HTML files. There are many different publishing options including including drag-and-drop, command line, or automated publishing from GitHub and GitLab repositories. Additionally, Netlify offers many helpful features such as website previews in pull requests. See the Netlify documentation (https://docs.netlify.com/) or the RStudio webinar "Sharing on Short Notice" (https://rstudio.com/resources/webinars/sharing-on-short-notice-how-to-get-your-materials-online-with-r-markdown/) for more details. 

### Advice

While all of the above options are good choices for sharing your work, there are numerous reasons to favor Netlify. It is a flexible option which is compatible with a wide variety of tools. It also allows you to have a portable workflow if you later decide to change services. These benefits are described in more detail on Yihui's blog (https://yihui.org/en/2017/06/netlify-instead-of-github-pages/) and the ["Deployment" chapter](https://bookdown.org/yihui/blogdown/deployment.html) of the **blogdown** book [@blogdown2017].
