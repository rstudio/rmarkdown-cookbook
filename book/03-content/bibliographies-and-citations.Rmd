...
Although beyond the scope of the book, it is worth highlighting at the behaviour of CSL files can be tweaked to customise the behaviour as required. For example, we can change the number of authors required before "et.al" is used to abbreviate them. This can be simplified through the use of visual editors such as the one found [here](http://editor.citationstyles.org/visualEditor/).
