...
1. The main author can accept suggested changes and download the Rmd document via `download_rmd()`, or render the changed document via `render_rmd()`. Other collaborators can also do this by themselves if they have modified code chunks and want to see the new results.

1. If satisfied, the main author can commit changes to the GIT repository, otherwise the Rmd document can be further revised locally, and updated in Google Drive via `update_rmd()`.

Please note that the collaborative editing can only be synchronous, i.e., only one person can update the document at a time. The main author has to coordinate the efforts of all collaborators, to avoid the situation of more than one person editing the document. If this constraint is not practical, you may have to consider tools for asynchronous collaboration instead, such as GIT. Using Google Drive only gives people who do not know GIT a chance to edit documents in a more user-friendly environment.
