...
To use R Markdown, you have to install R [@R-base] and the R package **rmarkdown** [@R-rmarkdown]. We also recommend that you install the RStudio IDE (https://www.rstudio.com). RStudio is not required, but it will make it easier for an average user to work with R Markdown because of the editor support. If you choose not to use the RStudio IDE, you will need to install Pandoc\index{Pandoc} (https://pandoc.org), which is the tool used by **rmarkdown** to convert Markdown to other document formats.
