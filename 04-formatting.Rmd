...
For **bookdown**'s `gitbook` output format, you may need to adjust the CSS a little bit for the line numbers to be displayed properly on the left side of the code. Below is what we used for this book (if you find the line numbers too close to the left margin, increase the `left` value to, say, `-2em`):
...
In order to automatically add line numbers to chunks, set `knitr::opts_hooks` and `knitr::opts_chunk`.
A following example automatically adds line numbers to code blocks of source, output, message, warning, and error.
If you want to add line numbers to certain types of code blocks generated by chunks,
specify your choice to the `numberLines` chunk option (e.g., `numberLines=c('source','output')`).
Specifying `numberLines=NULL` removes line numbers completely.

```{r, eval=FALSE}
# A function to generate hooks
numberLines <- function(code = c("source", "output", "message", "warning", "error")) {
  .code <- match.arg(code)
...
# Automatically fire hooks by fix attr.* to a blank string (or any but `NULL`)
knitr::opts_chunk$set(
  ...
  attr.warning = "",
  attr.error = "",
  numberLines = c("source", "output", "message", "warning", "error")
)
```
