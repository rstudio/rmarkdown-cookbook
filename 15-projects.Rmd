...
We recommend that you use the argument `local = knitr::knit_global()` explicitly in `source()` or `envir = knitr::knit_global()` explicitly in `sys.source()` to make sure the code is evaluated in the correct environment. The default value for the `local` or `envir` argument respectively of `source()` or `sys.source()` may not be the appropriate environment: you may end up creating variables in the wrong environment, and being surprised that why certain objects are not found in later code chunks.
