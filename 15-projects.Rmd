...
We recommend that you use the argument `envir = knitr::knit_global()` explicitly in `source()` or `sys.source()` to make sure the code is evaluated in the correct environment. The default value for the `envir` argument of `source()` or `sys.source()` may not be the appropriate environment: you may end up creating variables in the wrong environment, and being surprised that why certain objects are not found in later code chunks.
...
There is a disadvantage of the `source()` method in Section \@ref(source-script). That is, you will not be able to see the source code by default. You can use `source(..., echo = TRUE)`, but the source code will not be properly syntax highlighted. Besides, you need to be careful about the `envir` argument of `source()`, as mentioned in Section \@ref(source-script). In this section, we introduce an alternative method that does not have these problems.
