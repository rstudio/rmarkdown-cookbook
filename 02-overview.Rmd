...
We can them access this section with many of the tools that you will learn by referencing it by name or by applying options to all sections that are tagged with the same class. As examples, Section \@ref(cross-ref) demonstrates how to use section names to make cross-references throughout your document, and Section \@ref(html-tabs) introduces the `.tabset` class to help reorganize subheaders.
