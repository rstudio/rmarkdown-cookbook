...
Certain R Markdown templates will allow you to specify additional parameters directly within the YAML. For example, the [Distill](https://rstudio.github.io/distill/) output format allows `url`, `affiliation`, and `affiliation_url` to be specified. We must first install the **distill** package:
...
````md
`r ''````{r chunk-a, ref.label=c('chunk-c', 'chunk-b')}
```
...
````md
`r ''````{r get-labels, echo = FALSE}
labs = knitr::all_labels()
labs = setdiff(labs, c("setup", "get-labels"))
```

`r ''````{r all-code, ref.label=labs, eval=FALSE}
```
````
