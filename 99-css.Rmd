...
This document assumes a basic knowledge of CSS concepts including selection by tag, class, or id. You may find a quick crash course in CSS in the [CSS chapter](https://bookdown.org/yihui/blogdown/css.html) of the **blogdown** book [@blogdown2017] or study more in many free online tutorials such as https://www.w3schools.com/css/.
...
- Sections created by Pandoc Divs (`:::`) are fenced by `<div>` tags. These can be given a class. For example, `::: MyDiv` and `::: {.MyDiv}` translate to `<div class = 'MyDiv'>` (If this is a new concept to you, please see Sections \@ref(multi-column) and \@ref(custom-blocks) for examples)
