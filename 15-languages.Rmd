...
The R package **sass** [@R-sass] can be used to compile Sass to CSS. Based on the **sass** package, **knitr** includes two language engines: `sass` and `scss` (corresponding to the Sass and SCSS syntax, respectively) to compile code chunks to CSS. Below is a `scss` code chunk, with the chunk header ```` ```{scss}````:
