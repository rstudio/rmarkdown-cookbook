...
check_has_asymptote <- (system("which asy", intern = TRUE) == 0)
check_ci <- is.na(Sys.getenv('CI', NA))
eval_asy <- (check_not_windows & check_has_asymptote & check_ci)
if (!eval_asy) warning("System set-up not compatible with Asymptote, so chunks with asy engine will be skipped.")
```
